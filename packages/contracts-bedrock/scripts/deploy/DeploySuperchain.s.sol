...
contract DeploySuperchain is Script {
    ...
    function deploySuperchainProxyAdmin(InternalInput memory _input, Output memory _output) private {
        // Deploy the proxy admin, with the owner set to the deployer.
        ...
        vm.startBroadcast(_input.deployer);
        IProxyAdmin superchainProxyAdmin = IProxyAdmin(
            DeployUtils.create1({
                _name: "ProxyAdmin",
                _args: DeployUtils.encodeConstructor(abi.encodeCall(IProxyAdmin.__constructor__, (_input.deployer)))
            })
        );
        vm.stopBroadcast();
    ...
    function toInternalInput(Input memory _input) internal pure returns (InternalInput memory input_) {
        input_ = InternalInput(
            ...
            address(0), // The deployer is set in the run function.
            _input.paused,
